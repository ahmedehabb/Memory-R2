...
#
# Two-node setup:
#   Node 0 (server node) — launches a vLLM OpenAI-compatible server hosting
#                          the judge model.
#   Node 1 (client node) — waits for the server to be ready, then runs
#                          training using the local vLLM server as judge.
#
# HOW TO RUN (with salloc interactive session):
#
#   # 1. Allocate 2 nodes (adjust partition/gres to your cluster):
#   salloc --nodes=2 --ntasks-per-node=1 --gres=gpu:4 --time=24:00:00
#
#   # 2. From the shell you land in (node 0), launch on BOTH nodes via srun:
#   srun --ntasks=2 --ntasks-per-node=1 bash vllm_two_node.sh
#
#   srun sets SLURM_NODEID=0 on node 0 (vLLM server)
#             SLURM_NODEID=1 on node 1 (training)
#   The script auto-detects the role from SLURM_NODEID.
#   When training finishes, node 1 writes a done-file and the server exits.
#
# For sbatch (non-interactive):
#   sbatch --nodes=2 --ntasks=2 --ntasks-per-node=1 --gres=gpu:4 vllm_two_node.sh
# =============================================================================
...
# ---------------------------------------------------------------------------
# Model to serve as the judge. Can be any model supported by vLLM.
VLLM_JUDGE_MODEL=${VLLM_JUDGE_MODEL:-"openai/gpt-oss-120b"}
VLLM_PORT=${VLLM_PORT:-8000}
# GPUs dedicated to the vLLM server on the server node.
VLLM_TENSOR_PARALLEL=${VLLM_TENSOR_PARALLEL:-4}
# Max model length for the judge (tune to your GPU memory).
...
kl_loss_coef=0.01
REPLAY_MIX_RATIO=0
EXPERIMENT_NAME="${JOB_ID}_vllm_judge_${MAX_SESSIONS}sess"

# ---------------------------------------------------------------------------
# 3. Determine which node this process is running on
# ---------------------------------------------------------------------------
# With SLURM: SLURM_NODEID=0 is the server node, =1 is the client node.
# Without SLURM: set IS_SERVER=1 on the server machine and IS_SERVER=0 on client.
if [ -n "$SLURM_NODEID" ]; then
    NODE_ID=$SLURM_NODEID
else
    # Fallback: user sets IS_SERVER=1 on the server node explicitly.
    NODE_ID=${IS_SERVER:-0}
fi

# Resolve the server node hostname.
# With SLURM we parse SLURM_NODELIST; otherwise SERVER_HOST must be set manually.
if [ -n "$SLURM_NODELIST" ]; then
    # scontrol returns nodes one per line when expanded; pick the first (index 0).
    SERVER_HOST=$(scontrol show hostname "$SLURM_NODELIST" | head -n 1)
else
    SERVER_HOST=${SERVER_HOST:-localhost}
fi

VLLM_BASE_URL="http://${SERVER_HOST}:${VLLM_PORT}/v1"

echo "============================================================"
echo " NODE_ID      : $NODE_ID"
echo " SERVER_HOST  : $SERVER_HOST"
echo " VLLM_BASE_URL: $VLLM_BASE_URL"
echo " JUDGE_MODEL  : $VLLM_JUDGE_MODEL"
echo "============================================================"
...
# ---------------------------------------------------------------------------
# 5. SERVER NODE (NODE_ID == 0): launch vLLM server via Apptainer
# ---------------------------------------------------------------------------
if [ "$NODE_ID" -eq 0 ]; then
    if [ ! -f "$SIF_PATH" ]; then
        echo "[server] ERROR: SIF not found at $SIF_PATH — run setup_judge_env.sh first."
        exit 1
    fi
    ...
    echo "[server] Starting vLLM server (cc=$COMPUTE_CAPABILITY) for: $VLLM_JUDGE_MODEL"
    TIKTOKEN_DIR=<workspace>/tiktoken_encodings
    apptainer run --nv \
    ...
    echo "[server] vLLM server exited."
    exit $?
fi

# ---------------------------------------------------------------------------
# 6. CLIENT NODE (NODE_ID == 1): wait for server, then train
# ---------------------------------------------------------------------------
conda activate rema  # training env — untouched
echo "[client] Waiting for vLLM server at $VLLM_BASE_URL ..."
...
while true; do
    if curl -sf "${VLLM_BASE_URL}/models" > /dev/null 2>&1; then
        echo "[client] vLLM server is ready."
        break
    fi
    if [ "$elapsed" -ge "$WAIT_TIMEOUT" ]; then
        echo "[client] ERROR: vLLM server did not become ready within ${WAIT_TIMEOUT}s. Aborting."
        exit 1
    fi
    echo "[client] ... still waiting (${elapsed}s elapsed)"
    sleep "$WAIT_INTERVAL"
    elapsed=$((elapsed + WAIT_INTERVAL))
done

# Point judge_llm.py at the local vLLM server instead of Together/OpenAI.
# JUDGE_PROVIDER=openai uses the OpenAI SDK which auto-reads OPENAI_BASE_URL.
export JUDGE_PROVIDER=openai
export OPENAI_BASE_URL="$VLLM_BASE_URL"
export OPENAI_API_KEY="EMPTY"   # vLLM does not require a real key
export OPENAI_JUDGE_MODEL="$VLLM_JUDGE_MODEL"

# ---------------------------------------------------------------------------
# From here: exact copy of curriculum_learning.sh
# (env/key exports are skipped — already set above; JUDGE_PROVIDER kept as openai)
# ---------------------------------------------------------------------------
