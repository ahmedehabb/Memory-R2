...
# Single-agent ablation (G4): one model per turn receives raw dialogue turns
# + current memory state and directly produces INSERT/UPDATE/DELETE without
# the intermediate fact-extraction step.
#
# Baseline: inner_n8_rerun (two-agent, turns=4, val=0.488, mfail=0.050)
# Fair comparison: same MAX_NUM_TURNS=4 (session chunks), same 8 sessions.
# In two-agent mode turns=4 means 4 meta + 4 executor calls per conv.
# In single-agent mode turns=4 means 4 combined calls per conv — fewer FLOPs
# per conv, but this is the architectural ablation point (is the 2-agent split
# itself beneficial, or can one model do both steps equally well?).
#
# Run on the training node:
#   srun --jobid=<training_job_id> bash "$SCRIPT_DIR/vllm_client_8sess_single_agent.sh"
...
# ---------------------------------------------------------------------------
# Single-agent ablation training (8 sessions, same config as inner_n8_rerun)
# ---------------------------------------------------------------------------
export PROJECT_NAME="rema-ablations"
export BASE_MODEL="Qwen/Qwen2.5-7B-Instruct"
CURRENT_MODEL_PATH=$BASE_MODEL

# Matched to inner_n8_rerun for fair comparison:
#   MAX_NUM_TURNS=4 (same session chunk count)
#   num_rollouts=16, max_prompt_length=24576, max_response_length=2048
#   8 sessions, 10 epochs, inner GRPO fraction=0.5
MAX_NUM_TURNS=4
NUM_TRAIN_CONVS=1
NUM_VAL_CONVS=1
NUM_TEST_CONVS=8
...
echo "STARTING single-agent ablation: $STAGE_SESSIONS Sessions, $STAGE_EPOCHS Epochs"

export STAGE_EXP_NAME="8sess_single_agent_turns${MAX_NUM_TURNS}_${PPO_EPOCHS}ppo_Kl${kl_loss_coef}_${REWARD_TYPE}_${COMPRESSION_PENALTY}addcomp_turn_${algorithm}_${NUM_TRAIN_CONVS}convs${num_rollouts}r_innergrpo${INNER_GPRO_FRAC}"
...
PYTHONUNBUFFERED=1 python -m verl.rema_trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.single_agent_mode=true \
...
echo "[client] Single-agent ablation training complete."
echo "[client] Results logged to: $LOG_DIR/${STAGE_EXP_NAME}.log"
echo "[client] Compare val/acc and mfail against two-agent baseline: inner_n8_rerun (val=0.488, mfail=0.050)"
