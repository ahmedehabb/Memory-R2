...
# N4 — 32-sess single-agent curriculum continuation.
# Continues from the 16-sess single-agent checkpoint (`8sess_single_agent_turns4_..._step10`).
# Same MAX_NUM_TURNS=4, same hyperparams as the 8-sess single-agent run for fair comparison.
#
# Run on the training node:
#   srun --jobid=<training_job_id> bash "$SCRIPT_DIR/vllm_client_16sess_single_agent.sh"
...
export JOB_ID=${JOB_ID:-${SLURM_JOB_ID:-single_agent_32sess_$(date +%Y%m%d_%H%M%S)}}
...
while true; do
    ...
    [ "$found" -gt 0 ] && break
    if [ "$elapsed" -ge "$SERVER_WAIT_TIMEOUT" ]; then echo "[client] ERROR: no servers"; exit 1; fi
...
for f in $SERVER_FILES; do
    ...
    if curl -sf "${URL}/models" >/dev/null 2>&1; then
        BASE_URLS="${BASE_URLS}${BASE_URLS:+,}${URL}"
    fi
done
[ -z "$BASE_URLS" ] && { echo "[client] ERROR: no healthy server"; exit 1; }
echo "[client] using $BASE_URLS"
...
# ---------------------------------------------------------------------------
# 16-sess single-agent continuation training
# ---------------------------------------------------------------------------
export PROJECT_NAME=${PROJECT_NAME:-"rema-curriculum-v1"}
# Use the 8-sess single-agent step10 ckpt as the starting point
SA8_CKPT="/hkfs/work/workspace/scratch/tum_eyi5958-myspace2/projects/ReMA-public/checkpoints/rema-curriculum-v1/16sess_single_agent_n4_turns4_2ppo_Kl0.001_persession_0.2addcomp_turn_grpo_1convs8r_innergrpo0.5/global_step_5/hf_fixed"
...
STAGE_SESSIONS=32
STAGE_EPOCHS=5  # short continuation (curriculum convention)
export REMA_REWARD_QA_TOP_K_PER_SPEAKER=30

echo "[client] STARTING N4 16-sess single-agent continuation: from $CURRENT_MODEL_PATH"

export STAGE_EXP_NAME="32sess_single_agent_n4_turns${MAX_NUM_TURNS}_${PPO_EPOCHS}ppo_Kl${kl_loss_coef}_${REWARD_TYPE}_${COMPRESSION_PENALTY}addcomp_turn_${algorithm}_${NUM_TRAIN_CONVS}convs${num_rollouts}r_innergrpo${INNER_GPRO_FRAC}"
...
echo "[client] N4 16-sess single-agent continuation done. Log: $LOG_DIR/${STAGE_EXP_NAME}.log"
