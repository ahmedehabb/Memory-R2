...
# Source repo .env so HF_TOKEN / OPENAI_API_KEY survive across srun (srun doesn't propagate non-exported user env)
set -a
source /hkfs/work/workspace/scratch/tum_eyi5958-myspace2/projects/ReMA-public/.env 2>/dev/null || true
set +a
...
# ---------------------------------------------------------------------------
# Judge / answer-agent server
#   USE_OPENAI_API_JUDGE=1 -> use the real OpenAI API directly (no local server),
#     model from OPENAI_JUDGE_MODEL (default gpt-4o-mini). Otherwise discover a
#     local vLLM judge server via the rendezvous dir (original behaviour).
# ---------------------------------------------------------------------------
WAIT_INTERVAL=5
if [ "${USE_OPENAI_API_JUDGE:-0}" = "1" ]; then
    export JUDGE_PROVIDER=openai
    export JUDGE_BASE_URLS="${JUDGE_BASE_URLS:-https://api.openai.com/v1}"
    export JUDGE_API_KEY="${OPENAI_API_KEY:?Set OPENAI_API_KEY via env or sourced .env file}"
    export OPENAI_API_KEY="${OPENAI_API_KEY}"
    export OPENAI_JUDGE_MODEL="${OPENAI_JUDGE_MODEL:-gpt-4o-mini}"
    echo "[test-eval] Using real OpenAI API judge: model=$OPENAI_JUDGE_MODEL url=$JUDGE_BASE_URLS"
else
...
# Optional: dump per-QA (question, gold, prediction) records to JSONL for
# post-hoc LLM-judge scoring. Set REMA_DUMP_QA=1 and REMA_QA_DUMP_DIR to enable.
# Only emits for splits listed in REMA_QA_DUMP_SPLITS (default test,val).
if [ -n "${REMA_DUMP_QA:-}" ]; then
  ...
  echo "[test-eval] QA-dump enabled: dir=$REMA_QA_DUMP_DIR splits=$REMA_QA_DUMP_SPLITS run=$REMA_RUN_NAME"
fi
