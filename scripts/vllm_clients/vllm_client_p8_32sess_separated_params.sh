...
CLIP_RATIO=0.2
INNER_GPRO_FRAC=0.5
# P8 Base/Trained or Trained/Base: SWITCH_FREQ=200 effectively never switches → frozen role stays frozen.
SWITCH_FREQ=${SWITCH_FREQ:-200}
START_AGENT=${START_AGENT:-reasoning}  # default = P8 Base/Trained (train reasoning, meta frozen)
MODEL_PATH_META=${MODEL_PATH_META:?P8 launcher requires MODEL_PATH_META env (path to meta_thinking/hf_fixed)}
MODEL_PATH_REASONING=${MODEL_PATH_REASONING:?P8 launcher requires MODEL_PATH_REASONING env (path to reasoning/hf_fixed)}
P8_TAG=${P8_TAG:-base_trained}
...
export STAGE_EXP_NAME="32sess_p8_${P8_TAG}_switch${SWITCH_FREQ}_start${START_AGENT}_turns${MAX_NUM_TURNS}_${PPO_EPOCHS}ppo_Kl${kl_loss_coef}_${REWARD_TYPE}_${COMPRESSION_PENALTY}addcomp_turn_${algorithm}_${NUM_TRAIN_CONVS}convs${num_rollouts}r_innergrpo${INNER_GPRO_FRAC}"
...
PYTHONUNBUFFERED=1 python -m verl.rema_separated_trainer.main_ppo \
    ...
    actor_rollout_ref.actor.ppo_max_token_len_per_gpu=${PPO_MAX_TOKEN_LEN:-$((max_prompt_length+max_response_length))} \
