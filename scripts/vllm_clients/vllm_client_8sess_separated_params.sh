...
CLIP_RATIO=0.2
INNER_GPRO_FRAC=0.5
# Switch every 10 steps: agent0 trains steps 0-9, agent1 trains steps 10-19, etc.
SWITCH_FREQ=10
...
export STAGE_EXP_NAME="8sess_separated_params_switch${SWITCH_FREQ}_turns${MAX_NUM_TURNS}_${PPO_EPOCHS}ppo_Kl${kl_loss_coef}_${REWARD_TYPE}_${COMPRESSION_PENALTY}addcomp_turn_${algorithm}_${NUM_TRAIN_CONVS}convs${num_rollouts}r_innergrpo${INNER_GPRO_FRAC}"
...
PYTHONUNBUFFERED=1 python -m verl.rema_separated_trainer.main_ppo \
    ...
    algorithm.switch_agent.model_paths=[$BASE_MODEL,$BASE_MODEL] \
    algorithm.switch_agent.start_agent=meta_thinking \
...
echo "[client] Compare val/acc and mfail against shared-params baseline: inner_n8_rerun (val=0.488, mfail=0.050)"
