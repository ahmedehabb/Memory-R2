"""
ReMA-adapted memory search pipeline for LongMemEval.

Loads per-conversation Memory objects saved by add.py, searches for memories
relevant to each question, then calls answerBot to produce an answer.

Expected memory files:
  <memory_store_dir>/longmemeval_item_{idx}.pkl  (fast path, preferred)
  <memory_store_dir>/longmemeval_item_{idx}.json (fallback)
"""
...
# ---------------------------------------------------------------------------
# Answer prompt (minimal, single-user version for LongMemEval)
# ---------------------------------------------------------------------------
ANSWER_PROMPT = """
...
2. Pay special attention to timestamps to determine the chronological order of events.
3. If the question involves relative time references ("last year", "two months ago", etc.),
   convert them to absolute dates using the memory timestamp as the reference point.
4. If memories contain contradictory information, prioritise the most recent one.
...
def _answer_extraction(text) -> str:
    """Extract the answer from the model response. Tolerates None / non-string."""
    if text is None:
        return ""
...
def _load_memory(memory_store_dir: str, sample_id: str,
    ...
    """Load a Memory object from disk (pickle preferred, JSON fallback)."""
    ...
    if pkl_path.exists():
        ...
        # JSON fallback: re-insert each memory so embeddings are computed.
...
class MemorySearch:
    """
    Answers LongMemEval questions by searching per-conversation ReMA Memory objects.
    """

    def __init__(
        self,
        output_path: str = "results/rema_results.json",
    ...
    def _search_memory(self, mem: Memory, query: str) -> list:
        """Return top-k memories relevant to query as formatted strings."""
        t0 = time.time()
        results = mem.search(
            query,
            top_k=self.top_k,
        ...
        for mem_dict, score in results:
            ...
            # Format: "session_time — Speaker: content" matching how locomo pipeline does it
    ...
    def process_data_file(self, file_path: str) -> None:
        ...
        for idx, item in tqdm(enumerate(data), total=len(data), desc="Answering questions"):
            ...
            answer, memories_used, search_time, response_time = self.answer_question(mem, question)
