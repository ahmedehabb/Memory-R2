...
def create_device_mesh(world_size, fsdp_size):
    if fsdp_size < 0 or fsdp_size >= world_size:
        ...
        device_mesh = init_device_mesh('cuda',
                                       mesh_shape=(world_size // fsdp_size,
                                                   fsdp_size),
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if rollout_name == 'hf':
            ...
            rollout = HFRollout(module=self.actor_module_fsdp,
            ...
            log_gpu_memory_usage(f'Before building {rollout_name} rollout',
                                 logger=None)
            ...
            if vllm_mode == 'customized':
                ...
                raise NotImplementedError(
                    "vllm_mode must be 'customized' or 'spmd'")
            log_gpu_memory_usage(f'After building {rollout_name} rollout',
                                 logger=None)
    ...
    def init_model(self):
        from verl.workers.actor import DataParallelReMAPPOActor
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelReMAPPOActor(
        ...
        if self._is_ref:
            self.ref_module_fsdp = self._build_model_optimizer(
                model_path=self.config.ref_model.path,
                fsdp_config=self.config.ref.fsdp_config,
                optim_config=None,
                override_model_config=override_model_config,
                use_remove_padding=use_remove_padding,
                trust_remote_code=self.config.ref_model.get(
                    'trust_remote_code', False),
                use_liger=self.config.ref_model.get('use_liger', False),
                role='ref')[0]
            OmegaConf.set_struct(self.config.ref, True)
            with open_dict(self.config.ref):
                self.config.ref.use_remove_padding = use_remove_padding
            self.ref_policy = DataParallelReMAPPOActor(
        ...
        if self._is_actor:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(
                ...
                if self.processor is not None else self.tokenizer,
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics[
                'perf/max_memory_allocated_gb'] = torch.cuda.max_memory_allocated(
                ) / (1024**3)
            metrics[
                'perf/max_memory_reserved_gb'] = torch.cuda.max_memory_reserved(
                ) / (1024**3)
            metrics['perf/cpu_memory_used_gb'] = psutil.virtual_memory(
            ).used / (1024**3)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        meta_info = {
            'eos_token_id':
            self.generation_config.eos_token_id if self.generation_config
            is not None else self.tokenizer.eos_token_id,
            'pad_token_id':
            self.generation_config.pad_token_id if self.generation_config
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def raw_generate_sequences(self, prompts: DataProto):
        ''' generate_sequences_without any sharding and offloading '''
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def enter_generate_context(self, prompts: DataProto):
        assert self._is_rollout
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def exit_generate_context(self, prompts: DataProto):
    ...
    def multi_turn_generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            output = self.rollout.multi_turn_generate_sequences(
                prompts=prompts,
                tokenizer=self.tokenizer,
                max_num_turns=self.config.rollout.max_num_turns,
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(
                ...
                # tensors={f'{role}_old_log_probs': output[role] for role in output},
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        self.tokenizer = hf_tokenizer(tokenizer_path,
                                      trust_remote_code=config.model.get(
                                          'trust_remote_code', False))
        self.processor = hf_processor(tokenizer_path,
                                      trust_remote_code=config.model.get(
                                          'trust_remote_code', False))
        ...
        if num_warmup_steps < 0:
            num_warmup_steps_ratio = config.optim.get('lr_warmup_steps_ratio',
                                                      0.)
            num_warmup_steps = int(num_warmup_steps_ratio * total_steps)
    ...
    def update_critic(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.critic_module)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(optimizer=self.critic_optimizer,
                                device_id=torch.cuda.current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(
                global_num_tokens, delta_time)
            metrics[
                'perf/mfu/critic'] = estimated_flops * self.config.ppo_epochs / promised_flops / self.world_size
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda',
            ...
            step_ids = micro_batch['step_ids']
            labels = micro_batch['labels']

            if self.use_remove_padding:
                raise NotImplementedError("Remove padding is not implemented for reward model")
                ...
                position_ids_rmpad = index_first_axis(
                    rearrange(position_ids.unsqueeze(-1),
                              "b s ... -> (b s) ..."),
            ...
            # this is because labels is for next-token prediction, but rm predict
            # the reward for current token
            # prepend -100 to labels and then silce with labels[:, :-1]
            right_shift_labels = torch.cat([labels.new_full((batch_size, 1), -100), labels[:, :-1]], dim=1)
            rm_score[right_shift_labels == -100] = 0.0

            ###################################################################
            # YOU SHOULD CHECK HERE for n_head=2 classifier
            # 
            #
            #
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            input_ids, attention_mask = verl_F.tokenize_and_postprocess_data(
                ...
                truncation=self.config.get('truncation',
        ...
        with self.ulysses_sharding_manager:
            ...
            if use_dynamic_bsz:
                ...
                assert len(indices) == scores.size(
                    0), f"{len(indices)} vs. {scores.size()}"
                revert_indices = torch.tensor(get_reverse_idx(indices),
                                              dtype=torch.long)
