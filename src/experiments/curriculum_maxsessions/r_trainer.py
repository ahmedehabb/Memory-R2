        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # Accumulation Logic
                    max_sessions = self.config.trainer.get('max_sessions', 1) # Default to 1 if not set
                    ...
                    print(f"[STEP {self.global_steps}] chunk_id={current_chunk_id}, max_sessions={max_sessions}")
                    ...
                    if current_chunk_id < max_sessions and current_chunk_id < total_chunks:
                        print(f"[STEP {self.global_steps}] Accumulating batch (session {current_chunk_id}/{max_sessions}, total_chunks={total_chunks})...")
                    ...
                    # If we are here, it matches the final session (or max_sessions=1)
                    print(f"[STEP {self.global_steps}] Final session ({current_chunk_id}/{max_sessions}) reached. Proceeding to update.")

                    # Compute rewards for the FINAL session
                    with _timer('reward', timing_raw):
                        print(f"\n[STEP {self.global_steps}] Computing rewards for Final Session...")
                        ...
                        for key in keys_to_pop:
                            ...
                            # We only want to keep 'turn_level_reward' keys in the batch for PPO
                            # Everything else is just a metric to be logged.
                            # The 'outcome_rewards' logic below handles the main reward.
                            
                            # If it is a scalar, log it.
                            if key in scalar_metric_keys or tensor_value.dim() == 0:
                                ...
                                # It's a tensor metric (like 'bleu', 'acc' if they are tensorized)
                                # We do NOT want to add these to new_batch.batch because previous batches don't have them
                                # and we don't need them for the PPO update step.
                                # So we just log the mean and discard the tensor.
                                mean_val = tensor_value.float().mean().item()
                                metrics[f'train/{key}'] = mean_val
                                
                                # Also log under critic/ to keep existing Wandb charts working
                                if key in ['acc', 'bleu', 'evidence']:
                                    metrics[f'critic/{key}'] = mean_val
                        
                        # Store rewards in the current batch (Session 5)
                        # And identify the "Outcome Reward" to propagate
                        # We use 'reasoning_turn_level_reward' as the main reward signal
                        outcome_rewards = None
                        agent_roles = new_batch.meta_info['agent_roles']

                        # Assuming 'reasoning_turn_level_reward' is the key
                        ...
                        if main_reward_key in reward_tensor_map:
                            ...
                            # Compute outcome reward per sample: Sum of rewards (GRPO style)
                            # Shape: (B, T) -> (B,)
                            outcome_rewards = current_rewards.sum(dim=-1)
                            
                            # Add to current batch
                            new_batch.batch[main_reward_key] = current_rewards
                            
                            # Add other reward keys if any
                            for k, v in reward_tensor_map.items():
                                if k != main_reward_key and k.endswith('_turn_level_reward'):
                                    new_batch.batch[k] = v
                                    
                            # Compute turn level return for current batch (Session 5)
                            for role in agent_roles:
                                role_reward_key = f'{role}_turn_level_reward'
                                if role_reward_key in reward_tensor_map:
                                    # Ensure it's in the batch (might have been added above, but safe to ensure)
                                    new_batch.batch[role_reward_key] = reward_tensor_map[role_reward_key]
                                    
                                    turn_mask = verl_F.get_turn_mask(reward_tensor_map[role_reward_key], new_batch.non_tensor_batch['num_turns'])
                            ...
                            print(f"[WARNING] '{main_reward_key}' not found in reward_tensor_map. Propagation might fail.")
                            # Fallback: add all
                            for k, v in reward_tensor_map.items():
                                if k.endswith('_turn_level_reward'):
                                    new_batch.batch[k] = v

                        # Propagate rewards to accumulated batches (dense per-session rewards)
                        if 'per_session_f1' in reward_tensor_map:
                            per_session_f1_tensor = reward_tensor_map['per_session_f1'] # (bsz, max_sessions)
                            print(f"[STEP {self.global_steps}] Propagating dense per-session rewards to {len(self.accumulated_batches)} accumulated batches...")
                            
                            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                            max_turns = self.config.actor_rollout_ref.rollout.max_num_turns

                            # Create a mapping from (sample_id, rollout_idx) to reward_row_index
                            # for the current (final) session's reward tensor.
                            # rollout_idx is stored in new_batch.batch['rollout_idx']
                            # sample_id is stored in new_batch.non_tensor_batch['sample_id']
                            reward_mapping = {}
                            ...
                            # 1. Update accumulated batches (sessions 1 to max_sessions-1)
                            for i, prev_batch in enumerate(self.accumulated_batches):
                                ...
                                sess_idx = int(sess_idx) - 1 # 0-indexed: Session 1 is index 0, Session 2 is index 1, etc.
                                ...
                                for role in agent_roles:
                                    ...
                                    for row_idx_prev in range(bsz_prev):
                                        ...
                                        if key in reward_mapping:
                                            ...
                                            print(f"[WARNING] Key {key} not found in reward_mapping. Skipping reward for Session {sess_idx+1}.")
                            ...
                            for role in agent_roles:
                                ...
                                num_turns_curr = new_batch.non_tensor_batch['num_turns']
                                for j in range(len(new_batch.batch)):
                                    # Since we are in the current batch, we just use per_session_f1_tensor[j, final_sess_idx]
                                    # because the rewards in the manager were computed parallel to this batch.
                                    r = per_session_f1_tensor[j, final_sess_idx].item()
                                    last_turn_idx = num_turns_curr[j] - 1
                                    if last_turn_idx >= 0 and last_turn_idx < max_turns:
                                        curr_propagated_reward[j, last_turn_idx] = r
                                
                                new_batch.batch[role_reward_key] = curr_propagated_reward
                                turn_mask_curr = verl_F.get_turn_mask(curr_propagated_reward, num_turns_curr)
                                key_return_curr = role_reward_key.replace('reward', 'return')
                                new_batch.batch[key_return_curr] = core_algos.compute_turn_level_return(
                                    curr_propagated_reward, turn_mask_curr, self.config.algorithm.gamma_turn_level)

                        elif outcome_rewards is not None:
                            print(f"[STEP {self.global_steps}] Propagating Global average rewards to {len(self.accumulated_batches)} accumulated batches (Fallback)...")

                            for i, prev_batch in enumerate(self.accumulated_batches):
                                prev_sample_ids = prev_batch.non_tensor_batch['sample_id']
                                bsz_prev = len(prev_sample_ids)
                                max_turns_prev = self.config.actor_rollout_ref.rollout.max_num_turns
                                prev_num_turns = prev_batch.non_tensor_batch['num_turns']
                                
                                # Use mapping for global fallback as well if possible, or assume same order if outcome_rewards is just a vector
                                # Actually outcome_rewards is from new_batch, so we should map it too.
                                reward_mapping_global = {}
                                curr_sample_ids = new_batch.non_tensor_batch['sample_id']
                                curr_rollout_idxs = new_batch.batch['rollout_idx']
                                if isinstance(curr_rollout_idxs, torch.Tensor):
                                    curr_rollout_idxs = curr_rollout_idxs.cpu().tolist()
                                for idx in range(len(new_batch.batch)):
                                    reward_mapping_global[(curr_sample_ids[idx], int(curr_rollout_idxs[idx]))] = outcome_rewards[idx].item()

                                # We propagate the SAME outcome reward to ALL roles
                                for role in agent_roles:
                                    role_reward_key = f'{role}_turn_level_reward'
                                    propagated_reward = torch.zeros((bsz_prev, max_turns_prev), device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                                    
                                    prev_rollout_idxs = prev_batch.batch['rollout_idx']
                                    if isinstance(prev_rollout_idxs, torch.Tensor):
                                        prev_rollout_idxs = prev_rollout_idxs.cpu().tolist()

                                    for j in range(bsz_prev):
                                        key = (prev_sample_ids[j], int(prev_rollout_idxs[j]))
                                        if key in reward_mapping_global:
                                            outcome_r = reward_mapping_global[key]
                                            last_turn_idx = prev_num_turns[j] - 1
                                            if last_turn_idx >= 0 and last_turn_idx < max_turns_prev:
                                                propagated_reward[j, last_turn_idx] = outcome_r
