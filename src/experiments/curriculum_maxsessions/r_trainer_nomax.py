        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # Accumulation Logic (Ragged Batching)
                    # We accumulate ALL batches and trigger a single mega-update at the end of the epoch.
                    # This handles variable length sessions naturally.
                    self.accumulated_batches.append(batch)
                    print(f"[STEP {self.global_steps}] Accumulated batch. Total batches: {len(self.accumulated_batches)}")

                    # Immediate Reward Calculation & Propagation for Finished Trajectories
                    # Check if any sample in the current batch has valid QA pairs (meaning it reached the end of a session)
                    has_qas = False
                    if 'qa_pairs_json' in batch.non_tensor_batch:
                         for qa_json in batch.non_tensor_batch['qa_pairs_json']:
                             # Check if list is not empty
                             if len(json.loads(qa_json)) > 0:
                                 has_qas = True
                                 break
                    
                    if has_qas:
                        print(f"\n[STEP {self.global_steps}] Found finished trajectories (QAs present). Computing rewards...")
                        
                        # We need to compute rewards for the current batch
                        # Note: We pass the WHOLE batch to reward_fn, but it might only return scores for finished ones?
                        # REMA manager computes scores for all. If unfinished (no QAs), it presumably returns 0 or empty?
                        # Let's rely on REMA's behaviour.
                        
                        # Configure reward manager meta info
                        batch.meta_info['mask_unfinished_reward'] = self.config.reward_model.mask_unfinished_reward
                        batch.meta_info['use_format_reward'] = self.config.reward_model.get('use_format_reward', False)

                        with _timer('reward', timing_raw):
                            reward_tensor_map = self.reward_fn(batch)
                        
                        print(f"[STEP {self.global_steps}] Reward computed. Keys: {list(reward_tensor_map.keys())}")

                        # --- Metric Logging (Unchanged) ---
                        category_names = ['multi_hop', 'single_hop', 'temporal', 'open_domain', 'adversarial']
                        for cat_name in category_names:
                            f1_sum_key = f'{cat_name}_f1_sum'
                        ...
                        # Store rewards in the current batch
                        # We assume 'reasoning_turn_level_reward' is the main key
                        main_reward_key = 'reasoning_turn_level_reward'
                        agent_roles = batch.meta_info['agent_roles']
                        
                        # Initialize outcome_rewards to None to avoid UnboundLocalError later
                        outcome_rewards = None

                        if main_reward_key in reward_tensor_map:
                            current_rewards = reward_tensor_map[main_reward_key]
                            # Capture outcome rewards (sum over turns) if available
                            outcome_rewards = current_rewards.sum(dim=-1)
                            batch.batch[main_reward_key] = current_rewards
                            
                            # Add other reward keys
                            for k, v in reward_tensor_map.items():
                                if k != main_reward_key and k.endswith('_turn_level_reward'):
                                    batch.batch[k] = v
                            
                            # Compute turn level return for CURRENT batch
                            for role in agent_roles:
                                role_reward_key = f'{role}_turn_level_reward'
                                if role_reward_key in reward_tensor_map:
                                    batch.batch[role_reward_key] = reward_tensor_map[role_reward_key]
                                    turn_mask = verl_F.get_turn_mask(reward_tensor_map[role_reward_key], batch.non_tensor_batch['num_turns'])
                                    key_return = role_reward_key.replace('reward', 'return')
                                    batch.batch[key_return] = core_algos.compute_turn_level_return(
                                        reward_tensor_map[role_reward_key], turn_mask, self.config.algorithm.gamma_turn_level)
                        else:
                             print(f"[WARNING] '{main_reward_key}' not found in reward_tensor_map.")
                             for k, v in reward_tensor_map.items():
                                if k.endswith('_turn_level_reward'):
                                    batch.batch[k] = v
                        ...
                        if 'per_session_f1' in reward_tensor_map:
                            per_session_f1_tensor = reward_tensor_map['per_session_f1'] # (bsz, current_session_depth)
                            # print(f"[STEP {self.global_steps}] Propagating rewards backwards to {len(self.accumulated_batches)} accumulated batches...")

                            # Map (sample_id, rollout_idx) -> row_index in CURRENT batch
                            reward_mapping = {}
                            curr_sample_ids = batch.non_tensor_batch['sample_id']
                            curr_rollout_idxs = batch.batch['rollout_idx']
                            if isinstance(curr_rollout_idxs, torch.Tensor):
                                curr_rollout_idxs = curr_rollout_idxs.cpu().tolist()
                            
                            for row_idx in range(len(batch.batch)):
                                key = (curr_sample_ids[row_idx], int(curr_rollout_idxs[row_idx]))
                                reward_mapping[key] = row_idx

                            # Iterate over ALL accumulated batches (history)
                            # Note: self.accumulated_batches INCLUDES the current 'batch' (appended above)
                            # But we usually only need to update the PREVIOUS batches logic here?
                            # Actually, we should update ALL matching samples in history logic.
                            # INCLUDING the current batch if we want to be consistent, but we already set current batch above.
                            # Let's iterate and update any matching sample that ISNT the current batch (or overwrite? Overwrite is safe).
                            
                            for prev_batch in self.accumulated_batches:
                                bsz_prev = len(prev_batch.batch)
                                prev_num_turns = prev_batch.non_tensor_batch['num_turns']
                                prev_sample_ids = prev_batch.non_tensor_batch['sample_id']
                                prev_rollout_idxs = prev_batch.batch['rollout_idx']
                                if isinstance(prev_rollout_idxs, torch.Tensor):
                                    prev_rollout_idxs = prev_rollout_idxs.cpu().tolist()
                                
                                # Identify which session this batch belongs to
                                sess_idx = prev_batch.non_tensor_batch['chunk_id'][0]
                                if isinstance(sess_idx, torch.Tensor):
                                    sess_idx = sess_idx.item()
                                sess_idx = int(sess_idx) - 1 # 0-indexed
                                
                                # Check if this session index is valid for our reward tensor
                                # per_session_f1_tensor shape is (B, max_sessions_for_this_batch)
                                # If sess_idx >= tensor.shape[1], we can't update (mismatch or future chunk?)
                                ...
                                for role in agent_roles:
                                    ...
                                    if role_reward_key in prev_batch.batch:
                                        propagated_reward = prev_batch.batch[role_reward_key]
                                    else:
                                        propagated_reward = torch.zeros((bsz_prev, max_turns), device=device)
                                    
                                    for row_idx_prev in range(bsz_prev):
                                        key = (prev_sample_ids[row_idx_prev], int(prev_rollout_idxs[row_idx_prev]))
                                        
                                        if key in reward_mapping:
                                            row_idx_curr = reward_mapping[key]
                                            
                                            # Check if we have a reward for this session index
                                            if sess_idx < per_session_f1_tensor.shape[1]:
                                                r = per_session_f1_tensor[row_idx_curr, sess_idx].item()
                                                
                                                last_turn_idx = prev_num_turns[row_idx_prev] - 1
                                                if last_turn_idx >= 0 and last_turn_idx < max_turns:
                                                    propagated_reward[row_idx_prev, last_turn_idx] = r
                                                    if role == agent_roles[0]: updated_count += 1
                                            
                                    prev_batch.batch[role_reward_key] = propagated_reward
                                    turn_mask_prev = verl_F.get_turn_mask(propagated_reward, prev_num_turns)
                                    key_return_prev = role_reward_key.replace('reward', 'return')
                                    prev_batch.batch[key_return_prev] = core_algos.compute_turn_level_return(
                                        propagated_reward, turn_mask_prev, self.config.algorithm.gamma_turn_level)

                                # print(f"   Updated {updated_count} samples in batch (Chunk {sess_idx+1})")
                        
                        else:
                            # If no per-session F1, we propagate the global outcome reward (scalar) to all previous chunks
                            # This ensures that earlier parts of the conversation get the final reward signal.
                            
                            print(f"[STEP {self.global_steps}] Propagating Global Outcome Reward to {len(self.accumulated_batches)} accumulated batches...")
                            if outcome_rewards is None:
                                # Fallback if outcome_rewards wasn't computed above
                                if main_reward_key in new_batch.batch:
                                    outcome_rewards = new_batch.batch[main_reward_key].sum(dim=-1)
                                else:
                                    outcome_rewards = torch.zeros(len(new_batch.batch), device=device)

                            # Map (sample_id, rollout_idx) -> reward_value
                            reward_val_mapping = {}
                            curr_sample_ids = new_batch.non_tensor_batch['sample_id']
                            curr_rollout_idxs = new_batch.batch['rollout_idx']
                            if isinstance(curr_rollout_idxs, torch.Tensor):
                                curr_rollout_idxs = curr_rollout_idxs.cpu().tolist()
                            
                            for row_idx in range(len(new_batch.batch)):
                                key = (curr_sample_ids[row_idx], int(curr_rollout_idxs[row_idx]))
                                reward_val_mapping[key] = outcome_rewards[row_idx].item()

                            # Iterate over accumulated batches
                            for i, prev_batch in enumerate(self.accumulated_batches):
                                bsz_prev = len(prev_batch.batch)
                                prev_num_turns = prev_batch.non_tensor_batch['num_turns']
                                prev_sample_ids = prev_batch.non_tensor_batch['sample_id']
                                prev_rollout_idxs = prev_batch.batch['rollout_idx']
                                ...
                                for role in agent_roles:
                                    ...
                                    # Ensure tensor exists (Init with zeros if missing)
                                    if role_reward_key not in prev_batch.batch:
                                        prev_batch.batch[role_reward_key] = torch.zeros((bsz_prev, max_turns), device=device, dtype=torch.float32)
                                    
                                    propagated_reward = prev_batch.batch[role_reward_key] # (B, T)
                                    
                                    updated_count = 0
                                    for row_idx_prev in range(bsz_prev):
                                        key = (prev_sample_ids[row_idx_prev], int(prev_rollout_idxs[row_idx_prev]))
                                        if key in reward_val_mapping:
                                            r = reward_val_mapping[key]
                                            # Assign to last turn of this chunk
                                            last_turn_idx = prev_num_turns[row_idx_prev] - 1
                                            if last_turn_idx >= 0 and last_turn_idx < max_turns:
                                                propagated_reward[row_idx_prev, last_turn_idx] = r
                                                updated_count += 1
                                ...
                                # print(f"   [Batch {i}] Propagated outcome reward to {updated_count} samples")


                    # Since we iterate `for batch_dict in self.train_dataloader:`, this loop ends naturally.
                    # But we need to execute the update logic.
                    # We can check if `is_last_step` is True relative to total steps? 
                    # No, `is_last_step` is global. We need 'is_last_batch_of_epoch'.
                    # We can infer it: we are inside the loop. The loop will exit.
                    # BUT `fit()` continues?
                    # The structure is `for batch_dict in ...`.
                    # We should perform the update AFTER the loop?
                    # But `ray_trainer.py` prints "TRAINING STEP" inside logic.
                    # If we wait until after loop, we lose the per-step structure?
                    # Actually, for PPO, 'step' usually means 'update step'.
                    # If we update once per epoch, we have 1 step per epoch.
                    # The current `global_steps` increments inside the loop.
                    # We should move the update logic to be triggered conditionally or consolidate.
                    
                    # Check for End of Epoch (Mega-Update)
                    # We trigger update ONLY when we are at the very last batch of the epoch.
                    # We check if we have accumulated all batches expected in the dataloader.
                    
                    is_last_batch_in_epoch = (len(self.accumulated_batches) == len(self.train_dataloader))
                    
                    if is_last_batch_in_epoch:
                        print(f"\n[STEP {self.global_steps}] End of Epoch reached. Performing Mega-Update...")

                        # --- CRITICAL: Ensure ALL accumulated batches have reward keys before CONCAT ---
                        # This runs once per epoch, ensuring consistency even if the last batch had no QAs.
                        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                        max_turns = self.config.actor_rollout_ref.rollout.max_num_turns
                        agent_roles = batch.meta_info['agent_roles'] # batch is available from loop

                        reward_keys_to_ensure = []
                        for role in agent_roles:
                            reward_keys_to_ensure.append(f'{role}_turn_level_reward')
                            reward_keys_to_ensure.append(f'{role}_turn_level_return')
                        
                        for batch_idx, acc_batch in enumerate(self.accumulated_batches):
                            bsz_acc = len(acc_batch.batch)
                            for r_key in reward_keys_to_ensure:
                                if r_key not in acc_batch.batch:
                                    acc_batch.batch[r_key] = torch.zeros((bsz_acc, max_turns), device=device, dtype=torch.float32)

                        # Concatenate everything
                        if self.accumulated_batches:
                            print(f"[STEP {self.global_steps}] Concatenating {len(self.accumulated_batches)} batches...")
                            mega_batch = DataProto.concat(self.accumulated_batches)
                            self.accumulated_batches = [] # Clear accumulation
                            print(f"[STEP {self.global_steps}] Mega-batch size: {len(mega_batch.batch)}")

                            with _timer('adv', timing_raw):
                                print(f"[STEP {self.global_steps}] Computing advantages (Trajectory Aggregated)...")
                                merged_batch = merge_roles_data(mega_batch)
                                ...
                                mega_batch = merged_batch
                                mega_batch.batch['token_level_rewards'] = mega_batch.batch['token_level_scores']

                                print(f"[STEP {self.global_steps}] Computing advantages with {self.config.algorithm.adv_estimator}...")
                                mega_batch = compute_advantage(mega_batch,
                            ...
                            if self.use_critic:
                                with _timer('update_critic', timing_raw):
                                    critic_output = self.critic_wg.update_critic(mega_batch)
                            ...
                            if self.config.trainer.critic_warmup <= self.global_steps:
                                with _timer('update_actor', timing_raw):
                                    actor_output = self.actor_rollout_wg.update_actor(mega_batch)
                                actor_output_metrics = reduce_metrics(actor_output.meta_info['metrics'])
                                metrics.update(actor_output_metrics)
                            
                            # Log update metrics
                            print(f"[STEP {self.global_steps}] Mega-Update Complete.")
                        
                        else:
                            print("[STEP {self.global_steps}] Warning: No batches to update.")

                        # Increment global_steps HERE, because 1 update = 1 step (epoch)
                        self.global_steps += 1
                        print(f"[FIT] Incrementing global_steps to {self.global_steps}")

                        # --- Periodic Validation and Checkpointing (Moved inside Update Block) ---
                        ...
                        # collect metrics (Summary for the epoch) -> Moved outside _timer block
                        pass
                    
                    batch = None
                    # Reset num_gen_batches at end of epoch? No, loop rebuilds it.
                    # But we are inside loop.
                # Compute and log metrics only if we did a mega-update (end of epoch)
                if is_last_batch_in_epoch:
                    print(f"\n[STEP {self.global_steps}] Computing and logging metrics...")
                    # For throughout, we use the current mega_batch? Or accumulated?
                    # compute_data_metrics likely works on current 'batch'. But we want sum over epoch?
                    # Or just last batch stats?
                    # Using 'mega_batch' for metrics calculation ensures we reflect the whole update data.
                    metrics.update(compute_data_metrics(batch=mega_batch, use_critic=self.use_critic))
                    metrics.update(compute_timing_metrics(batch=mega_batch, timing_raw=timing_raw))
                    n_gpus = self.resource_pool_manager.get_n_gpus()
                    metrics.update(compute_throughout_metrics(batch=mega_batch, timing_raw=timing_raw, n_gpus=n_gpus))

                    logger.log(data=metrics, step=self.global_steps)
                    print(f"[STEP {self.global_steps}] Metrics logged. Step (Epoch) complete.\n")
