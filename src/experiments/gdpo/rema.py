...
class ReMARewardManager:
    ...
    def __call__(self, data: DataProto)-> Dict[str, torch.Tensor]:
        ...
        # Initialize both aggregated and per-category turn_level_rewards
        # Following original pattern: aggregated (for compute_token_level_scores) + components (for GDPO)
        category_names = ['multi_hop', 'single_hop', 'temporal', 'open_domain', 'adversarial']
        reward_tensor_map = {}
        
        # Initialize aggregated turn_level_rewards (sum of all categories)
        for role in agent_roles:
            reward_tensor_map[f'{role}_turn_level_reward'] = torch.zeros(batch_size, max_num_turns, dtype=torch.float32)
        
        # Initialize per-category turn_level_rewards for GDPO
        for role in agent_roles:
            for cat_name in category_names:
                reward_tensor_map[f'{role}_{cat_name}_turn_level_reward'] = torch.zeros(batch_size, max_num_turns, dtype=torch.float32)
        
        print(f"[RewardManager] Initialized reward_tensor_map with {len(reward_tensor_map)} rewards")
        print(f"[RewardManager] Aggregated keys: {[k for k in reward_tensor_map.keys() if not any(c in k for c in category_names)]}")
        print(f"[RewardManager] Per-category sample keys: {list(reward_tensor_map.keys())[-4:]}...")
        ...
        print(f"\n[RewardManager] Processing {len(data)} data items to assign per-category rewards...")
        for i_bsz in range(len(data)):
            data_item = data[i_bsz]  # DataProtoItem
            
            num_turns = data_item.non_tensor_batch['num_turns']
            
            # Get per-category scores from category_stats_list
            sample_category_stats = category_stats_list[i_bsz]
            
            # Process each question and assign to appropriate category reward
            ...
            for i_role, role in enumerate(agent_roles):
                ...
                # Initialize category scores dict
                category_scores = {cat_name: 0.0 for cat_name in ['multi_hop', 'single_hop', 'temporal', 'open_domain', 'adversarial']}
                
                # Assign scores per category based on QA pairs
                for qa_idx, qa_pair in enumerate(qa_pairs):
                    category_id = qa_pair.get('category', 0)
                    cat_name = CATEGORY_NAMES.get(category_id, 'unknown')
                    
                    # Get F1 score for this specific QA pair from category_stats
                    if category_id in sample_category_stats['f1_scores'] and qa_idx < len(sample_category_stats['f1_scores'][category_id]):
                        qa_f1_score = sample_category_stats['f1_scores'][category_id][qa_idx]
                    else:
                        qa_f1_score = 0.0
                    
                    # Check if masked (incomplete trajectory)
                    if data_item.meta_info['mask_unfinished_reward']:
                        turn_finished_val = data_item.batch[f'{agent_roles[0]}_turn_finished'].item()
                        if turn_finished_val != 1:
                            qa_f1_score = 0.0
                    
                    # Accumulate scores for this category (in case there are multiple questions per category)
                    category_scores[cat_name] += qa_f1_score
                
                # Assign accumulated category scores to respective reward tensors
                total_score = 0.0
                for cat_name, cat_score in category_scores.items():
                    reward_tensor_map[f'{role}_{cat_name}_turn_level_reward'][i_bsz, num_turns - 1] = cat_score
                    total_score += cat_score
                    
                    if i_bsz == 0 and cat_score > 0:
                        print(f"  - Assigned {role}_{cat_name}_turn_level_reward[{i_bsz}, {num_turns - 1}] = {cat_score}")
                
                # Assign aggregated reward (sum of all categories) - used by compute_token_level_scores
                reward_tensor_map[f'{role}_turn_level_reward'][i_bsz, num_turns - 1] = total_score
                if i_bsz == 0:
                    print(f"  - Assigned {role}_turn_level_reward[{i_bsz}, {num_turns - 1}] = {total_score} (sum of all categories)")
